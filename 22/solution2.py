...
def dependant(beneath, removed):
    return [brick for brick in beneath if all([x in removed for x in supporting_bricks(brick, beneath)]) and supporting_bricks(brick, beneath)]
...
with open("input.txt") as file:
    ...
    for brick in bricks:
        ...
        while next != removed:
            removed = next
            next = removed.union(dependant(beneath, removed))
        total += len(removed) - 1
